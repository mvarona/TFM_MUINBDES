...
# TFM MUINBDES
# 2022
# Returns the NSI code of a recommended municipality for a given input
...
def main(argv):
	if (len(sys.argv) < 3):
		error = "Error #1, llamada mal construida: " + str(sys.argv)
		response = build_error_response(error)
		return_response(response)
		sys.exit(2)
	...
	nsi_code = check_nsi_code_integrity(sys.argv[2])
